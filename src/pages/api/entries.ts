...
/**
 * POST /api/entries — upsert a journal entry.
 *
 * Accepts writes for today and any past date (editing an existing entry or
 * backfilling an empty day). Future dates are rejected with 403.
 *
...
export const POST: APIRoute = async ({ request }) => {
	...
	// Future dates are never writable; today and past dates are allowed
	// (editing an existing entry or backfilling an empty day).
	const today = todayISO(timezone);
	if (date > today) {
		return json({ error: 'Future entries cannot be written.' }, 403);
	}
